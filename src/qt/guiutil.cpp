...
{
    // ReferralCoin: check prefix
    if(uri.scheme() != QString("ReferralCoin"))
        return false;
...
{
    // Convert ReferralCoin:// to ReferralCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("ReferralCoin://"))
    {
        uri.replace(0, 12, "ReferralCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "ReferralCoin.lnk";
}
...
{
    return GetAutostartDir() / "ReferralCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("ReferralCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  ReferralCoin-qt [" + tr("command-line options") + "]                     " + "\n";
    ...
    setWindowTitle(tr("ReferralCoin-Qt"));
